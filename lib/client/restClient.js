...
module.exports = function RestClient(address, credential, ignoreCert) {
  ...
  this.post = function post(context, options, body) {
    context.log.info(options, `post to ${this.address}`);
    return this.request.post(options, body);
  };
  this.put = function put(context, options, body) {
    context.log.info(options, `put to ${this.address}`);
    return this.request.put(options, body);
  };
  this.patch = function patch(context, options, body) {
    context.log.info(options, `patch to ${this.address}`);
    return this.request.patch(options, body);
  };
